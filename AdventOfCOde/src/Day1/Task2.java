...
public class Task2 {
	...
	public static void main(String[] args) {
		...
		String[] spelledNumber = { "one", "two", "three", "four", "five", "six", "seven", "eight", "nine" };
		...
		for (String lineInput : splitInput) {

			List<Integer> lineNumber = new ArrayList<>();
			for (int i = 0; i < lineInput.length(); i++) {
				if (isDigit(lineInput.charAt(i))) {
					lineNumber.add(Integer.parseInt(String.valueOf(lineInput.charAt(i))));
				} else {
					for (int j = 0; j < spelledNumber.length; j++) {
						if (lineInput.substring(i).startsWith(spelledNumber[j])) {
							lineNumber.add(j + 1);
						}
					}
				}
			}
			sum += lineNumber.get(0) * 10 + lineNumber.get(lineNumber.size() - 1);
		}
